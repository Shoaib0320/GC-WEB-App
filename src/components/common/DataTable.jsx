    ...
    <motion.div initial="hidden" animate="visible" variants={fadeUp} className="space-y-4 w-full">
      ...
      {loading ? (
        ...
        <div className="w-full rounded-lg border bg-white shadow-sm overflow-hidden">
          ...
          {/* Main Table Container with Horizontal Scroll - FIXED */}
              ...
              <TableHeader className="bg-gray-50/80">
                <TableRow className="hover:bg-transparent">
                  {columns.map((col, i) => (
                    <TableHead 
                      key={i}
                      className={`px-3 sm:px-4 py-3 text-xs sm:text-sm font-semibold text-gray-700 whitespace-nowrap ${
                        col.align === "right" ? "text-right" : "text-left"
                      }`}
                      style={col.minWidth ? { minWidth: `${col.minWidth}px` } : {}}
                    >
                      {col.label}
                    </TableHead>
                  ))}
          ...
          {totalPages > 1 && (
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-t px-3 sm:px-4 py-3 bg-gray-50/50 text-xs sm:text-sm">
              {/* Showing info */}
              <div className="text-center sm:text-left text-gray-600 text-xs sm:text-sm">
                Showing <strong>{showingStart}</strong> to <strong>{showingEnd}</strong> of{" "}
                <strong>{totalItems}</strong> results
              </div>
              ...
              <div className="flex items-center justify-center sm:justify-end gap-2 flex-wrap">
                {/* Page info */}
                <div className="flex items-center gap-1 text-gray-600 whitespace-nowrap text-xs sm:text-sm">
                  <span className="hidden sm:inline">Page</span>
                  <span className="font-medium">{currentPage}</span>
                  <span>of</span>
                  <span className="font-medium">{totalPages}</span>
